...
describe('AudioTranslations test', () => {
  let client: Client;
  let audioTranscriptions: AudioTranslations;
  let mock: MockAdapter;

  beforeEach(() => {
    ({ client, mock } = unitClient());
    audioTranscriptions = new AudioTranslations(client);
  });
  ...
  describe('Unit test for `create` method', () => {
    ...
    it('should return promise with basic information', async () => {
      mock.onPost('/audio/translations').reply(200, {});
